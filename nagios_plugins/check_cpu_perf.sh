#!/bin/bash
#
# modificato da Merello il 27/1/2025, perchè la metrica CPUIdle non veniva riportata nel Performance Graph 
#  tolto dalla visualizzazione del valore della variabile CpuIdle la stringa :10:20 
...
if [ $SYSSTATRPM -gt 5 ]
 then
  ...
  CPU=`echo ${SARCPU}|awk '{print "CPU Idle = " $8 "% | " "CpuUser=" $3 "; CpuNice=" $4 "; CpuSystem=" $5 "; CpuIowait=" $6 "; CpuSteal=" $7 "; CpuIdle=" $8""}'`
 else
...
if [ $SYSSTATLSLPP -gt 4 ]
 then
  ...
  PERFDATA=`echo ${SARCPU}|awk '{print "CpuUser=" $2 "; CpuSystem=" $3 "; CpuIowait=" $4 "; CpuPhysc=" $6 "; CpuEntc=" $7 "; CpuIdle=" $5""}'`
...
if [ $SYSSTATPKGINFO -ge 11 ]
 then
  ...
  CPU=`echo ${SARCPU}|awk '{print "CPU Idle = " $5 "% | " "CpuUser=" $2 "; CpuSystem=" $3 "; CpuIowait=" $4 "; CpuIdle=" $5""}'`
 else
...
if [ $SYSSTATPKGINFO -ge 1 ]
 then
  ...
  CPU=`echo ${SARCPU}|awk '{print "CPU Idle = " $6 "% | " "CpuUser=" $2 "; CpuSystem=" $3 "; CpuNice=" $4 "; CpuIntrpt=" $5 "; CpuIdle=" $6""}'`
 else
